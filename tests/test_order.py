...
def test_read_nonexistent_order(client):
    ...
    response_read = client.get(
        f"/orders/{non_existent_id}", headers={"Authorization": f"Bearer {token}"}
    )
    assert response_read.status_code == 404
    data_read = response_read.json()
    assert (
        data_read["detail"]
        == "Order not found or you're not authorized to access this order"
    )
