...
async def add_staff(
    ...
    existing_ticket = session.exec(
        select(Ticket).where(
            Ticket.event_id == event.id, Ticket.attendee_id == staff.id
        )
    ).first()
    ...
    html_message = generate_staff_added_email(
        staff.name, event.name, event.date, event.location
    )
    ...
    return {"message": "Staff member added to event successfully", "event": event}
...
def delete_event(
    ...
    statement = (
        select(Event)
        .join(Ticket, Ticket.event_id == Event.id)
        .where(Event.id == event_id, Ticket.attendee_id == current_user.id)
    )
...
def check_in_attendee(
    ...
    if attendee.checked_in:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Attendee already checked in at "
...
@router.get(
    "/{event_id}/check-in-logs",
    response_model=CheckInLogList,
    status_code=status.HTTP_200_OK,
)
